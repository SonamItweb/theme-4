    ...
    <div class="content">
        <div class="container">
            <div class="col-md-10 col-md-offset-1">
                <div class="row">
                    <div class="col-sm-8 col-md-8 col-md-offset-2 contformbody">
                        <div class="msgs"></div>
                        <div class="emlerr"></div>
                        <div class="teleerr"></div>
                        <div class="text-center"> </div>
                        <form id="activityEnq" class="contactform" name="contactform" method="post">
                            <div class="row">
                            ...
                            <div class="row">
                                <div class="col-xs-12 col-sm-6">
                                    <div class="input-style-1 type-2 color-2">
                                        <input type="text" id="select-city-1" class="autocomplete validate" name="city" placeholder="Select City or Place" required>
                                    </div>
                                </div>
                                <div class="col-xs-12 col-sm-6">
                                    <div class="input-style-1 type-2 color-2">
                                        <select id="actList" name="activity" required>
                                            <option value="" disabled selected>Select Activity</option>
                                            <?php
                                                $query = mysql_query("select excursion_name from excursion_master_tariff where active_flag='Active'");
                                                while($row_ser = mysql_fetch_assoc($query)){

                                            ?>
                                            <option value="<?=  $row_ser['excursion_name'] ?>"><?=  $row_ser['excursion_name'] ?></option>
                                            <?php $count++; } ?>
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="row">
                                <div class="col-xs-12">
                                    <div class="input-style-1 type-2 color-2">
                                        <input type="text" id="from" name="arrival" placeholder="Arrival Date" autocomplete="off" required>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                            ...
                            <div class="row">
                                <div class="col-xs-12 text-center">
                                    <input type="submit" value="Book Now" class="frmBtn">
                                </div>
                            </div>
                        </form>
                    </div>
...
$mobile = selectQuery(VLIST,"*","(device='android' OR device='Windows Phone') AND device<>''"); ?-->
    ...
    $('#actList').select2();
    $("#from").datetimepicker({ timepicker:false, format:'d-m-Y' });

    $(document).ready(function(){
      $('#activityEnq').submit(function(e){
        // alert($("#activityEnq").serialize());
          $.ajax({
                        type: "POST",
                        url: "mail/activityEnquirySubmit.php",
                        data: $("#activityEnq").serialize(),
                        success: function(data) {
                           // location.reload();
                           // reset_form('activityEnq');
                           $('#activityEnq').html(data);
                        }
          });
          e.preventDefault();
        });
